...
def display_drug_card(drug, color):
    """Helper function to display a drug card with consistent styling"""
    ...
    condition_display = f"{condition[:60]}..." if len(condition) > 60 else condition
...
def render_competitor_pipeline():
    ...
    try:
        ...
        with st.spinner("Loading pipeline data..."):
            ...
            # Initialize pipeline_data as empty DataFrame if None
        ...
        if not pipeline_data.empty:
            ...
            # Reset index to start from 1 and name it
            ...
            styled_df = display_df.style.applymap(highlight_phase, subset=['Development Phase'])
